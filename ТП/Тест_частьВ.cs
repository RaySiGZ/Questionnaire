...
namespace ТП
{
    public partial class Тест_частьВ : Form
    {
        private bool[] ans_acc = new bool[] { false, false, false, false, false };
        private int ans_access = 0;
        private int ans_choice;
        private int ques_choice = 0;
        private Color[] backColor = new Color[5];
        public Тест_частьВ()
        {
            InitializeComponent();
        }

        private void Тест_частьВ_Load(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < 5; i++)
            {
                ...
                buttons[i].ForeColor = Назад.ForeColor;
                buttons[i].BackColor = Назад.BackColor;
                buttons[i+5].ForeColor = Color.FromArgb(100, Назад.ForeColor);
                buttons[i+5].BackColor = Color.FromArgb(100, Назад.BackColor); 
            }
        ...
        {
            ques_choice = 0;
            Button[] buttons = new Button[] { yt_Button1, yt_Button2, yt_Button3, yt_Button4, yt_Button5 };
            for (int i = 0; i < 5; i++) buttons[i].BackColor = backColor[i];
            buttons[ques_choice].BackColor = Color.FromArgb(150, backColor[ques_choice]);
        }
        ...
        private void Тест_частьВ_Close(object sender, FormClosingEventArgs e)
        {
            Form this_Логин = Application.OpenForms[2];
            this_Логин.Show();
        }
    }
}